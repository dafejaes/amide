...
class ControllerCertificate {
    ...
    public function certiget() {
        $q = "SELECT * FROM am_recepcion, am_equipos, am_equipos_tipo, am_ubicaciones, am_servicio WHERE rcp_borrado=0 AND rcp_estado = 'Terminado' AND am_equipos_eq_id = eq_id AND am_equipos_tipo_eqt_id=eqt_id AND am_servicio_ser_id = ser_id AND am_ubicaciones_ubi_id = ubi_id";
        if ($this->id > 0) {
